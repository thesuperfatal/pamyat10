    ...
    <div className="space-y-5">
      ...
      {step === "done" && (
        <div className="space-y-4">
          <section className="rounded-3xl border border-[var(--accent)]/40 bg-[var(--accent-soft)]/50 p-6 shadow-sm sm:p-7">
            <p className="text-xs font-semibold uppercase tracking-wide text-[var(--accent)]">
              {INTRO_DONE_STEP.eyebrow}
            </p>
            <h2 className="mt-2 font-[family-name:var(--font-display)] text-2xl font-semibold sm:text-3xl">
              {goal ? goal.leadsTo : INTRO_DONE_STEP.titleFallback}
            </h2>
            <p className="mt-3 text-sm leading-relaxed text-[var(--muted)] sm:text-base">
              {goal ? goal.why : INTRO_DONE_STEP.leadFallback}
            </p>

            <div className="mt-4 flex flex-wrap gap-2 text-xs">
              {goal ? (
                <span className="rounded-full bg-white/90 px-3 py-1 text-[var(--accent)] ring-1 ring-[var(--accent)]/20">
                  Цель: {goal.label}
                </span>
              ) : null}
              {tried ? (
                <span className="rounded-full bg-white/90 px-3 py-1 text-[var(--muted)] ring-1 ring-[var(--line)]">
                  Мини-проба пройдена
                  {tryOk === true ? " · верно" : tryOk === false ? " · с разбором" : ""}
                </span>
              ) : (
                <span className="rounded-full bg-white/90 px-3 py-1 text-[var(--muted)] ring-1 ring-[var(--line)]">
                  Пробу пропустили — нормально
                </span>
              )}
            </div>

            {lessonTip ? (
              <p className="mt-4 rounded-2xl bg-white/90 px-4 py-3 text-sm text-[var(--ink)]">
                <span className="font-medium text-[var(--accent)]">Приём на старт: </span>
                {lessonTip}
              </p>
            ) : null}
          ...
          <section className="rounded-3xl border border-[var(--line)] bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-end justify-between gap-2">
              <div>
                <h3 className="font-[family-name:var(--font-display)] text-lg font-semibold">
                  {INTRO_DONE_STEP.checklistTitle}
                </h3>
                <p className="mt-1 text-sm text-[var(--muted)]">{INTRO_DONE_STEP.checklistLead}</p>
              </div>
              <p className="text-xs text-[var(--accent)]">
                {Math.min(checks.size, checklist.length)} / {checklist.length}
              </p>
            </div>
            ...
            <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-[var(--bg-deep)]">
              <div
                ...
                style={{
                  width: `${(Math.min(checks.size, checklist.length) / checklist.length) * 100}%`,
                }}
              />
            </div>

            {checks.size >= checklist.length ? (
              <p className="mt-4 rounded-2xl bg-[var(--accent-soft)] px-4 py-3 text-sm text-[var(--accent)]">
                {INTRO_DONE_STEP.allDone}
              </p>
            ) : null}
          ...
          <section className="rounded-3xl border border-[var(--line)] bg-white p-5 shadow-sm">
            ...
            <ul className="mt-3 grid gap-2 sm:grid-cols-3">
              {INTRO_DONE_STEP.secondary.map((s) => (
                <li key={s.href}>
                  <Link
                    href={s.href}
                    className="block rounded-2xl border border-[var(--line)] bg-[var(--bg)] px-3 py-3 transition hover:border-[var(--accent)]"
                  >
                    <p className="text-sm font-medium text-[var(--ink)]">{s.label}</p>
                    <p className="mt-0.5 text-xs text-[var(--muted)]">{s.text}</p>
                  </Link>
                </li>
              ))}
            </ul>
